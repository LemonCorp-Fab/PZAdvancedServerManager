...
- dynamische Wiedererkennung lokaler Prozesse über das exakte Argument `-servername`, auch wenn der Server vor dem Manager gestartet wurde, mit Start-/Bereit-/Eingeschränkt-/Gestoppt-Phasen, begrenzter und bereinigter stdout/stderr-Erfassung, Live-Anzeige von `server-console.txt`, Netzwerkübersicht, authentifiziertem RCON-Status und Befehls-/Antwortkonsole; `save`/`quit` und koordinierter Neustart bleiben auch ohne SSH verfügbar;
...
6. SteamCMD mit einem Klick installieren, das Herausgeberkonto konfigurieren, **Sitzung verbinden / erneuern** ausführen und zuerst privat veröffentlichen.
