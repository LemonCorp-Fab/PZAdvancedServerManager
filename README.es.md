...
- redetección dinámica de procesos locales mediante el argumento exacto `-servername`, incluso si el servidor se inició antes que el gestor, con fases de inicio/listo/degradado/detenido, captura limitada y depurada de stdout/stderr, seguimiento en vivo de `server-console.txt`, vista de red, estado RCON autenticado y consola de comandos/respuestas; `save`/`quit` y el reinicio coordinado siguen disponibles, también sin SSH;
