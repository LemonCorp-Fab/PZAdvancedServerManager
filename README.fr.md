...
- redécouverte dynamique des processus locaux par l’argument exact `-servername`, y compris si le serveur précédait le manager, avec phases démarrage/prêt/dégradé/arrêté, capture bornée et expurgée de stdout/stderr, suivi en direct de `server-console.txt`, vue réseau, statut RCON authentifié et console commandes/réponses ; arrêt propre `save`/`quit` et redémarrage coordonné restent disponibles, y compris avec un profil distant sans SSH ;
