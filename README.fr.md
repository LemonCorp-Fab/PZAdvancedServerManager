...
- éditeur serveur guidé pour l’identité, l’accès, RCON, la session, les sauvegardes et le contenu, complété par l’éditeur INI brut avec préservation de l’encodage ; au démarrage local, la table SQLite `whitelist` est lue et le mot de passe initial de `admin` n’est demandé que si ce compte manque réellement ;
- redécouverte dynamique par `zombie.network.GameServer` et `-servername`, y compris si le serveur précédait le manager ; les processus `-coop` sont distingués des serveurs dédiés, le client graphique seul est ignoré et plusieurs instances partageant un profil sont signalées comme conflit. La vue serveur à onglets propose un journal `server-console.txt` ou `coop-console.txt` lisible, filtrable par sévérité et recherche, avec capture stdout/stderr bornée et expurgée, phases explicites, réseau, RCON et console commandes/réponses ;
...
L’UI écoute localement sur `http://127.0.0.1:5160`. Utilisez `--data-root <dossier>` pour partager explicitement les données entre l’UI et le CLI.
...
Sous Windows, `just docker-secret-setup` protège avec DPAPI le mot de passe initial et une clé de chiffrement indépendante hors du dépôt; les secrets RCON et API sont chiffrés au repos avec AES-GCM. Sous Linux, utilisez un `.env` en mode `600` ou un gestionnaire de secrets externe. Pour Coolify, ajoutez `PZASM_ADMIN_PASSWORD` et une clé stable `PZASM_DATA_ENCRYPTION_KEY` d’au moins 32 caractères aléatoires comme variables protégées; Compose les monte en fichiers secrets en lecture seule. Associez le port `5160` à un domaine HTTPS et conservez impérativement le volume `pzasm-data`. La procédure complète est décrite dans [Docker et Coolify](docs/DOCKER-COOLIFY.md).

## SteamCMD et serveurs distants

Pine Hosting dispose d'un backend API distinct. Une clé API et l'identifiant du serveur suffisent pour réutiliser les éditeurs INI, SandboxVars et Lua, le déploiement des packs, la console, le démarrage/arrêt et les sauvegardes fournisseur sans SSH. Les restaurations et fresh starts exigent un serveur arrêté et proposent une sauvegarde de sécurité préalable. La documentation technique complète est disponible dans [Pine Hosting provider](docs/PINE-HOSTING.md).
