...
SteamCMD es una dependencia gestionada, no un requisito manual. En la primera importación, actualización de fuentes, publicación, comprobación de sesión o reparación del servidor dedicado que lo necesite, PZASM usa un ejecutable configurado válido o descarga el archivo de Valve para la plataforma en `<data-root>/tools/steamcmd`, lo extrae de forma controlada, lo inicializa y reutiliza después la misma caché y sesión portátiles. Una ruta personalizada obsoleta vuelve automáticamente a la copia gestionada. Las descargas públicas del Workshop de Project Zomboid usan acceso anónimo de forma predeterminada. La interfaz transmite las fases de descarga, extracción, inicialización y verificación; cancelar también detiene el proceso de inicialización.
...
Con systemd, Docker, un panel de alojamiento u otro supervisor que reinicie Project Zomboid tras `quit`, un perfil RCON-only puede coordinar la publicación: primero termina la subida al Workshop y después el gestor envía `save` y `quit`. SSH queda limitado a la gestión INI opcional o a un comando explícito que inicie solo el juego. Se rechazan `reboot`, `shutdown` y `poweroff` del host. El secreto RCON se conserva para automatizar estas operaciones, pero se cifra en reposo; proteja los datos y la clave de cifrado del despliegue.

## Taller de compatibilidad y resolución de conflictos

El editor de paquetes y la vista de despliegue del servidor comparten un analizador estático en caché. Lee las estructuras Build 42 efectivas (`common` más la mejor carpeta versionada compatible), `require`, `loadAfter`, `loadBefore`, `incompatible`, Mod IDs duplicados, rutas virtuales Lua/scripts/assets, dependencias de mapas y celdas `.lotheader` superpuestas. Los archivos distintos solo se comparan por hash tras detectar una ruta y tamaño compartidos; el contenido idéntico queda registrado como información resuelta.

El taller propone un orden topológico estable para mods y mapas, muestra las pruebas exactas y permite elegir un ganador prioritario, aceptar una colisión intencionada o desactivar una fuente. Las prioridades manuales se convierten en restricciones de orden explícitas y nunca reescriben archivos de terceros. La auditoría del servidor también correlaciona el paquete con `WorkshopItems`, `Mods`, `Map` y fallos recientes del registro. El análisis estático no puede demostrar la compatibilidad de Lua arbitrario, por lo que las pruebas en juego siguen siendo obligatorias.

Una infracción de orden causada por una dependencia fuerte es bloqueante. Los componentes fuertemente conexos aíslan únicamente los mods del ciclo real, sin incluir todos los mods posteriores. Cuando un ciclo procede solo de un ganador de colisión manual que contradice `require`, `loadAfter` o `loadBefore`, el taller puede repararlo con un clic: elimina únicamente la restricción manual cuya invalidez está demostrada, reconstruye y valida el grafo y aplica el orden topológico estable. Si la validación sigue fallando, restaura las restricciones eliminadas. Los ciclos formados únicamente por restricciones declaradas por las fuentes siguen siendo bloqueos de resolución manual.

Las colisiones de archivos también se clasifican por impacto en ejecución: traducciones y medios pasivos de bajo riesgo, interfaz del cliente de riesgo moderado, jugabilidad compartida o scripts de riesgo alto, y Lua del servidor o datos de mapas de riesgo crítico. El diagnóstico separa estos tipos, muestra la primera ruta virtual conflictiva en cada cabecera y permite abrir cada copia física tras verificar que sigue dentro de una instantánea de mod administrada.

Las colisiones de texto compatibles ofrecen un editor de diferencias de solo lectura. El administrador puede elegir dos mods fuente, intercambiar lados, ignorar espacios, cambiar entre vistas paralela y unificada, buscar, conservar solo los cambios con contexto y navegar por los bloques. El resaltado dentro de la línea muestra los caracteres exactos modificados. Las rutas se vuelven a validar antes de leer, se rechaza el contenido binario, cada archivo está limitado a 2 MiB y la vista a 12.000 líneas por lado.

La compatibilidad dispone de una pestaña de proyecto propia. El panel principal solo muestra un resumen compacto del estado y abre esa pestaña sin repetir el análisis. Las recetas por lotes son deliberadamente estrictas: pueden desactivar mods cuya estructura para la versión objetivo falta de forma verificada, desactivar entradas sin fuente o `mod.info` efectivo disponible y aplicar el orden calculado de mods y mapas. Cada lote muestra sus objetivos exactos, conserva los snapshots y deja las colisiones ambiguas para una revisión explícita.

## Importaciones conscientes de dependencias

Cada importación local o del Workshop se analiza antes de modificar el proyecto. El gestor normaliza los Mod ID `require=` leídos de `mod.info`, los compara con el paquete actual y enumera las dependencias ausentes en el diálogo de confirmación de la aplicación. El administrador puede añadir el mod seleccionado con todas las dependencias resolubles o añadir deliberadamente solo el mod seleccionado.

Las dependencias locales se asocian por Mod ID exacto. Para fuentes del Workshop, PZASM también lee la lista oficial **Required Items** del elemento; las recomendaciones nunca se consideran dependencias. La reparación con un clic aparece tanto en el diagnóstico de dependencia ausente como en la tarjeta del mod afectado. Un elemento hijo descargado solo se acepta cuando su `mod.info` efectivo proporciona realmente el Mod ID solicitado. Si no existe una fuente verificada, el gestor informa del ID no resuelto en lugar de adivinar. Las dependencias añadidas se colocan antes del mod que las requiere y se vuelve a validar todo el orden.

## Filtros de descubrimiento del Workshop

El navegador público del Workshop combina el orden de Steam Community con un filtrado determinista de los detalles públicos. La búsqueda puede dirigirse al título y la descripción o a uno solo. Se admiten varias etiquetas requeridas y excluidas, con coincidencia de todas o al menos una etiqueta requerida. Otros filtros abarcan fecha de publicación y actualización, SteamID64 del creador, suscripciones actuales y acumuladas, favoritos, vistas, tamaño mínimo/máximo, disponibilidad de imagen/descripción y estado ya añadido en el destino.

La profundidad es explícita: se inspeccionan una, tres o cinco páginas de Steam por lote. Los IDs candidatos se deduplican antes de solicitar los detalles públicos por lotes y las páginas se guardan brevemente en caché. Los filtros numéricos y de metadatos se aplican después del descubrimiento para conservar un comportamiento determinista aunque la página pública ignore un parámetro URL opcional.
