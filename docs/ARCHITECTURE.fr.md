...
SteamCMD est une dépendance gérée, pas un prérequis manuel. Lors du premier import, rafraîchissement, publish, contrôle de session ou entretien du serveur dédié qui en a besoin, PZASM utilise un exécutable configuré valide ou télécharge l’archive Valve adaptée dans `<data-root>/tools/steamcmd`, l’extrait de façon contrôlée, lance son initialisation puis réutilise ce même cache et cette même session portables. Un chemin personnalisé périmé rebascule automatiquement vers la copie gérée. Les téléchargements publics Project Zomboid utilisent la connexion anonyme par défaut. L’UI diffuse les phases de téléchargement, extraction, initialisation et vérification ; une annulation arrête aussi le processus d’initialisation.
...
## Filtres de découverte Workshop

Le navigateur Workshop public combine l’ordre de navigation Steam Community avec un filtrage déterministe de la réponse publique détaillée. La recherche peut viser le titre et la description, ou un seul de ces champs. Plusieurs tags requis et exclus sont acceptés, avec une correspondance de tous les tags requis ou d’au moins un. Les autres filtres couvrent l’âge de publication et de mise à jour, le SteamID64 du créateur, les abonnements actuels et cumulés, les favoris, les vues, la taille minimale/maximale, la présence d’une image ou description et l’état déjà ajouté dans la destination.

La profondeur est explicite : une, trois ou cinq pages de résultats Steam sont inspectées par lot du manager. Les IDs candidats sont dédupliqués avant la lecture groupée des détails publics et les pages parcourues sont brièvement mises en cache. Les filtres numériques et de métadonnées sont appliqués après la découverte Steam afin de rester déterministes même si la page Workshop publique ignore un paramètre URL facultatif.
