...
Le planificateur valide les droits et dépendances, actualise éventuellement les sources, remplace les snapshots, construit, publie et coordonne éventuellement le serveur par RCON. Une connexion supervisée transmet le mot de passe à SteamCMD par son entrée standard, sans l’enregistrer. Si Steam Guard protège le compte, l’UI demande le code actuel puis relance la connexion avec la commande officielle `set_steam_guard_code`, toujours par l’entrée standard. SteamCMD conserve ensuite son propre jeton dans son dossier portable ; les publications manuelles et planifiées utilisent uniquement cette session. Le manager ne mémorise que l’heure de la dernière vérification. Une session expirée échoue immédiatement en demandant une reconnexion au lieu d’attendre sur une invite invisible. L’UI diffuse la sortie SteamCMD en direct, applique un délai maximal et peut annuler le processus externe.
