...
SteamCMD 是由管理器负责的依赖项，不需要用户预先手动安装。第一次执行需要它的导入、来源更新、发布、会话检查或专用服务器修复时，PZASM 会优先使用有效的自定义可执行文件；否则会把 Valve 对应平台的官方压缩包下载到 `<data-root>/tools/steamcmd`，安全解压并完成初始化，之后继续复用同一个便携缓存和会话。失效的自定义路径会自动回退到托管副本。Project Zomboid 的公开 Workshop 内容默认匿名下载。界面会实时显示下载、解压、初始化和验证阶段；取消操作也会终止初始化进程。
