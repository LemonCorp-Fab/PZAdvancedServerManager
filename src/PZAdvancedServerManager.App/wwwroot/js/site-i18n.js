(() => {
    ...
    const rows = [
        ...
        ['Utilisé uniquement pour créer ou renouveler la session portable, puis oublié. Le code Steam Guard sera demandé dans le popup uniquement si Steam le réclame.', 'Used only to create or renew the portable session, then forgotten. The popup requests a Steam Guard code only if Steam requires it.', 'Se usa solo para crear o renovar la sesión portátil y luego se olvida. El popup solicita el código Steam Guard únicamente si Steam lo requiere.', 'Wird nur zum Erstellen oder Erneuern der portablen Sitzung verwendet und danach verworfen. Das Popup fragt den Steam-Guard-Code nur ab, wenn Steam ihn verlangt.', 'Usada somente para criar ou renovar a sessão portátil e depois esquecida. O popup solicita o código Steam Guard apenas se a Steam exigir.', '仅用于创建或更新便携会话，随后即被丢弃。仅当 Steam 要求时，弹窗才会请求 Steam Guard 验证码。'],
