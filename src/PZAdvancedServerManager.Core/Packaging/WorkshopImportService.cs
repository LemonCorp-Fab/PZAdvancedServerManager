    ...
    PackageProjectService projects)
{
    public async Task<WorkshopImportResult> ImportAsync(PackageProject project, ulong workshopId, CancellationToken cancellationToken = default)
    {
        ...
        var count = 0;
        foreach (var mod in imported) count += projects.AddWithDependencies(project, mod, allKnown);
        return new WorkshopImportResult(workshopId, count, imported.Select(x => x.ModId).ToArray(), download.SteamCmd.CombinedOutput);
    }
